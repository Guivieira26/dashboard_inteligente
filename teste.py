...
#DEBUG dados previstos estasticos

# import sys
# sys.path.insert(0, "src")
# import pandas as pd
# from ml.predictor import carregar_modelo, simular

# carregar_modelo()
# df = pd.read_parquet("data/merged.parquet")

# # Simula 3 cenários com mesmo dólar, marketing diferente
# for mkt in [-0.30, 0.0, 0.80]:
#     df_proj = simular(df, dolar_simulado=5.00, variacao_mkt_pct=mkt)
#     placa = df_proj[df_proj["produto"] == "Placa de Vídeo RTX"].iloc[0]
#     print(f"Marketing {mkt*100:+.0f}%  →  "
#           f"receita_projetada: R${placa['receita_projetada']:,.0f}  "
#           f"marketing_futuro: R${placa['marketing_futuro']:,.0f}  "
#           f"margem: R${placa['margem_projetada']:,.0f}")
...
for mkt in [-0.30, 0.0, 0.80]:
    ...
    placa = df_proj[df_proj["produto"] == "Placa de Vídeo RTX"].iloc[0]
    print(f"Marketing {mkt*100:+.0f}%  →  "
          f"receita: R${placa['receita_projetada']:,.0f}  "
          f"mkt_futuro: R${placa['marketing_futuro']:,.0f}  "
          f"margem: R${placa['margem_projetada']:,.0f}")